  ...
  return (
    <div className='flex justify-between p-2 md:p-4 bg-[#d1ccd1] border-b-[1px] border-b-[#020200]'>
        <div className='flex items-center gap-0 md:gap-2.5'>
            ...
            <p className='text-[#020200] text-xl md:text-4xl font-light font-serif'>Glow</p>
        </div>
        <ul className='flex items-center list-none gap-2 md:gap-12 font-light font-sans text-xs md:text-base'>
            <li onClick={()=>{setMenu("shop")}} className='hover:scale-110 transition ease-in-out'>
              <Link href="/shop">
                SHOP{menu==="shop"?<hr className='bg-[#020200] h-1'/>:<></>}
              </Link>
            </li>
            <li onClick={()=>{setMenu("face")}} className='hover:scale-110 transition ease-in-out'>
              <Link href="/face" element={<ShopCategory category="Face" />}>
                FACE{menu==="face"?<hr className='bg-[#020200] h-1'/>:<></>}
              </Link>
            </li>
            <li onClick={()=>{setMenu("body")}} className='hover:scale-110 transition ease-in-out'>
              <Link href="/body" element={<ShopCategory category="Body" />}>
                BODY{menu==="body"?<hr className='bg-[#020200] h-1'/>:<></>}
              </Link>
            </li>
            <li onClick={()=>{setMenu("hair")}} className='hover:scale-110 transition ease-in-out'>
        ...
        <div className='flex justify-evenly gap-3 items-center'>
            <Link href="/cart" element={<Cart/>}>
              <Image src={cartIcon} alt='cart-icon' width={40} height={40} className='hover:scale-110'/>
            </Link>
            <div className='flex justify-center items-center md:w-5 md:h-5 text-xs md:text-sm md:rounded-xl md:bg-[#020200] md:text-[#ffffff] text-[#020200] font-sans'>{getTotalCartItems()}</div>
